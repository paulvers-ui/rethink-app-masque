 ...
 */
package com.creatore.rethinkfork.net.go
...
import com.creatore.rethinkfork.data.AppConfig.Companion.DOH_INDEX
import com.creatore.rethinkfork.data.AppConfig.Companion.DOT_INDEX
import com.creatore.rethinkfork.data.AppConfig.Companion.FALLBACK_DNS_IF_NET_DNS_EMPTY
import com.creatore.rethinkfork.data.AppConfig.TunnelOptions
import com.creatore.rethinkfork.database.ConnectionTrackerRepository
import com.creatore.rethinkfork.database.DnsCryptRelayEndpoint
import com.creatore.rethinkfork.database.EventSource
import com.creatore.rethinkfork.database.EventType
import com.creatore.rethinkfork.database.ProxyEndpoint
import com.creatore.rethinkfork.database.Severity
import com.creatore.rethinkfork.net.doh.Transaction
import com.creatore.rethinkfork.service.BraveVPNService
import com.creatore.rethinkfork.service.BraveVPNService.Companion.FIRESTACK_MUST_DUP_TUNFD
import com.creatore.rethinkfork.service.BraveVPNService.Companion.NW_ENGINE_NOTIFICATION_ID
import com.creatore.rethinkfork.service.EventLogger
import com.creatore.rethinkfork.service.PersistentState
import com.creatore.rethinkfork.service.ProxyManager
import com.creatore.rethinkfork.service.ProxyManager.ID_WG_BASE
import com.creatore.rethinkfork.service.RethinkBlocklistManager
import com.creatore.rethinkfork.service.WireguardManager
import com.creatore.rethinkfork.ui.activity.AppLockActivity
import com.creatore.rethinkfork.util.Constants
import com.creatore.rethinkfork.util.Constants.Companion.MAX_ENDPOINT
import com.creatore.rethinkfork.util.Constants.Companion.ONDEVICE_BLOCKLIST_FILE_TAG
import com.creatore.rethinkfork.util.Constants.Companion.REMOTE_BLOCKLIST_DOWNLOAD_FOLDER_NAME
import com.creatore.rethinkfork.util.Constants.Companion.RETHINKDNS_DOMAIN
import com.creatore.rethinkfork.util.Constants.Companion.RETHINK_BASE_URL_MAX
import com.creatore.rethinkfork.util.Constants.Companion.RETHINK_BASE_URL_SKY
import com.creatore.rethinkfork.util.Constants.Companion.UNSPECIFIED_IP_IPV4
import com.creatore.rethinkfork.util.Constants.Companion.UNSPECIFIED_IP_IPV6
import com.creatore.rethinkfork.util.UIUtils.getAccentColor
import com.creatore.rethinkfork.util.Utilities
import com.creatore.rethinkfork.util.Utilities.blocklistDir
import com.creatore.rethinkfork.util.Utilities.blocklistFile
import com.creatore.rethinkfork.util.Utilities.isAtleastO
import com.creatore.rethinkfork.util.Utilities.isAtleastS
import com.creatore.rethinkfork.util.Utilities.isPlayStoreFlavour
import com.creatore.rethinkfork.util.Utilities.showToastUiCentered
import com.creatore.rethinkfork.util.Utilities.tob
import com.creatore.rethinkfork.util.Utilities.togb
import com.creatore.rethinkfork.util.Utilities.togs
import com.creatore.rethinkfork.util.Utilities.tos
import com.creatore.rethinkfork.wireguard.Config
...
class GoVpnAdapter : KoinComponent {
    ...
    suspend fun addTransport() {
        ...
        when (appConfig.getDnsType()) {
            ...
            AppConfig.DnsType.RETHINK_REMOTE -> {
                // Only RethinkDNS has a separate block-free transport; add it first because
                // the Go-side ALG expects BlockFree before Preferred.
                val rdnsRemoteUrl = appConfig.getRemoteRethinkEndpoint()?.url
                val blockfreeUrl = appConfig.getBlockFreeRethinkEndpoint()

                // Block-free transport is optional — missing it is not fatal.
                if (blockfreeUrl.isNotEmpty()) {
                    ...
                    // Bug fix: original code returned early here, skipping the Preferred
                    // transport entirely. This caused repeated 'no rdns remote url found'
                    // errors and the DNS watchdog firing at 45 s on every cold start.
                    Logger.w(LOG_TAG_VPN, "$TAG no blockfree url — continuing to add Preferred")
                }
                ...
                if (!rdnsRemoteUrl.isNullOrEmpty()) {
                    ...
                    // rdnsRemoteUrl is null on first boot before the DB populates the endpoint.
                    // fallbackToRethinkDefault() is guarded against re-entrant calls.
                    Logger.e(LOG_TAG_VPN, "$TAG no rdns remote url found — invoking safe fallback")
    ...
    private suspend fun fallbackToRethinkDefault() {
        // Guard against re-entrant calls. handleRethinkChanges() calls addTransport() which —
        // when rdnsRemoteUrl is still null — would loop back here indefinitely, producing the
        // repeated 'no rdns remote url found' log spam seen in production.
        if (!fallbackInFlight.compareAndSet(false, true)) {
            Logger.w(LOG_TAG_VPN, "$TAG fallbackToRethinkDefault already in flight, skipping")
            return
        }
        try {
            ...
            if (rethinkDefault == null || rethinkDefault.url.isEmpty()) {
                Logger.e(LOG_TAG_VPN, "$TAG connect-tunnel: fallback; rethink default is empty")
                showDnsFailureToast("")
                return
            }
            // Directly register the transport instead of calling handleRethinkChanges()
            // to avoid re-entering the full addTransport() chain.
            Logger.i(LOG_TAG_VPN, "$TAG fallback: adding rethink default url: ${rethinkDefault.url}")
            addRdnsTransport(Backend.Preferred, rethinkDefault.url)
        } finally {
    ...
    private suspend fun setWireguardTunnelModeIfNeeded(tunProxyMode: AppConfig.TunProxyMode) {
        ...
        // fix: double-check the user selection; wireguard proxies must not be started when
        // wireguard is not the selected proxy (stale active config rows in db)
        if (!appConfig.isWireGuardEnabled()) {
            Logger.i(LOG_TAG_VPN, "$TAG wireguard not selected, skip adding wg proxies")
            return
        }
        ...
        Logger.i(LOG_TAG_VPN, "$TAG added wireguards: ${wgConfigs.size}")
        wgConfigs.forEach {
    ...
    // Guards fallbackToRethinkDefault() from re-entering addTransport() recursively.
    // Root cause: handleRethinkChanges() -> addTransport() -> fallback -> infinite loop.
    private val fallbackInFlight = java.util.concurrent.atomic.AtomicBoolean(false)
