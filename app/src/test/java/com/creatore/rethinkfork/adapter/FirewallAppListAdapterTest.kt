...
class FirewallAppListAdapterTest {
    ...
    fun setup() {
        ...
        // Always stop any existing Koin instance and start fresh with current mocks
        ...
        // Set main dispatcher AFTER Koin and Application are initialised so no
        // Application.onCreate() coroutines land on the test dispatcher before runTest starts.
    ...
    fun tearDown() {
        ...
        try { Dispatchers.resetMain() } catch (e: Exception) {
            println("⚠️ TEARDOWN: resetMain failed - ${e.message}")
        }
        try { unmockkAll() } catch (e: Exception) {
            println("⚠️ TEARDOWN: unmockkAll failed - ${e.message}")
        }
        try {
            stopKoin()
