...
@Config(sdk = [28], application = android.app.Application::class) // Use older SDK that works better with resource loading
class WelcomeActivityTest {
    ...
    @Before
    fun setUp() {
        // Get context FIRST — Application.onCreate() may call startKoin internally
        context = ApplicationProvider.getApplicationContext()

        // Stop whatever Koin instance is now running (ours or the Application's)
        try { stopKoin() } catch (_: Exception) { /* ignore if not running */ }

        mockPersistentState = mockk<PersistentState>(relaxed = true)
        every { mockPersistentState.firstTimeLaunch } returns true
    ...
    fun testPersistentState_MockBehavior() {
        ...
        every { mockPersistentState.firstTimeLaunch } returns false
        assertFalse("Mock should return false for firstTimeLaunch", mockPersistentState.firstTimeLaunch)

        // Verify mock interactions
        verify(atLeast = 2) { mockPersistentState.firstTimeLaunch }
    }
    ...
    fun testLaunchHomeScreen() {
        try {
            ...
            verify(exactly = 1) { mockPersistentState.firstTimeLaunch = false }
    ...
    fun testWelcomeActivity_MockInteractions() {
        ...
        // Simulate marking onboarding as complete (relaxed mock — setter is a no-op by default)
        mockPersistentState.firstTimeLaunch = false

        // Verify the interaction
        verify(atLeast = 1) { mockPersistentState.firstTimeLaunch }
    }
    ...
    fun testNextButtonClickOnLastPage() {
        try {
            ...
            if (viewPager != null && btnNext != null) {
                ...
                verify(atLeast = 1) { mockPersistentState.firstTimeLaunch = false }
            }
